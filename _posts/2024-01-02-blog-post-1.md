...
different than that presented before, as it introduces and addtional bias term. 
This is a very important detail as the bias is derived from each aiminoacid pair 
...
Adding a bias term to the colum-wise attention on the other hand would not make any sense, 
as columns do not represent a single protein structure, but rather equivalent positions
of different protein homologues. <br />
...
on its two neighbors, and "triangular self-attention" in which the update to a residue pair
<span class="math inline"><em>ij</em></span> is calculated using a query derived from 
...
<span class="math inline"><em>ik</em></span>.<br />
