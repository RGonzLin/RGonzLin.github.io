...
and <span class="math inline"><strong>x</strong></span> vectors are the embedding reprentations of the tokens (words in this case):  
<br>
<br>
...
MSA and a pairwise map (again, a sort of initial guess of residue positions in space). 
...
attention are calculated for the MSA. Row-wise attention would find relationships within the same protein
...
it is the mechanism by which it “leaks” information onto the MSA for its update.
...
colum pairs of the MSA to update the pairwise representation, a couple 
...
orientation.<br />
To understand how this is done, we need to introduce the concept of a 
“residue gas”. A protein is a chain of linked amino acids, each composed of
three backbone atoms (always a N, C^{\alpha}, and C'), and a side which varies
per specific amino acid species. The idea of AlphaFold is to construct one triangle
per residue whose vertices correspond to N, C^{\alpha}, and C', and detach each residue from 
their neigbhors. As the residues are now detached they are allowed to move freely, and 
hence the name residue gas. Each residue is parameterized in euclidian space
using a translation vector (t_{i}, the distance from the origin to the C^{\alpha} carbon), 
and a rotation matrix (R_{i}), as T_{i}=(t_{i},R_{i}).<br /> 
At first, all residues are initialized at the origin and they are free to move during inference.
It is the job of the structure modele, and in particular of the IPA, to update the position these 
triangles with the help of the pairwise representation comming from the evoformer.<br />
...
additionally introduces a euclidian-aware term, 
presented here in a slightly simplified way for the sake of clarity:
<p><span class="math display">$$
\text{Column-Wise Attention}_{\text{ij}}(\mathbf{q_i}, \mathbf{k_j}, \mathbf{v_j}, \mathbf{b_{ij}}, \mathbf{T_i}, \mathbf{T_j}) = \text{softmax}\left(\mathbf{q_i}^T \mathbf{k_j} + \mathbf{b_{ij}} + \| \mathbf{T_i q_i - T_j k_j} \|^{2}\right) \mathbf{v_j}
$$</span> 
Altough in principle it is posible residues contiguos in the linked linear chain end up far appart 
...
but this comprises a fairly complete explanation of the inner workings of AlphaFold2. 
