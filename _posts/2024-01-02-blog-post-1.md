...
![attentionBrown](../images/attentionBrown.png)
The attention for the word brown can be calculated in the following manner:</p>
<p><span class="math display">$$\text{Attention}(\mathbf{q_2}, \mathbf{K}, \mathbf{V}) = \text{softmax}\left(\frac{\mathbf{q_2K}^T}{\sqrt{d_K}}\right)\mathbf{V}$$</span>  
More generally, the attention for the whole sentence can be calulated in at once by collecting all 
queries into a query matrix class="math inline"><strong>Q</strong></span>.
