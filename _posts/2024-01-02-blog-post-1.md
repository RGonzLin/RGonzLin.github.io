...
ignoring some details like scaling (which are unecessary given the scope of the blog post), would then be:
...
on its two neighbors, and “triangular self-attention updates” in which the update to a residue pair
...
<span class="math inline"><em>ik</em></span>
<p><span class="math display">$$\text{Triangular Attention}_{\text{i}}(\mathbf{q_{ij}}, \mathbf{k_{ik}}, \mathbf{v_{ik}}, \mathbf{b_{jk}}) = \text{softmax}\left(\mathbf{q_{ij}}^T \mathbf{k_{ik}} + \mathbf{b_{jk}}\right) \mathbf{v_{ik}}$$</span>  
followed by an equivalent update around <span class="math inline"><em>i</em></span>. 
...
orientation.<br />
Another important idea related <br />
